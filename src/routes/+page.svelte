...
<form class="race1">
	...
	<InputBlock name="time">
		<TimeInput name="time" bind:value={() => time1seconds, (v) => (time1seconds = v)} withHours unit="" />
    </InputBlock>

	<InputBlock name="pace">
    {#if paceUnits == MIN_KM}
		<TimeInput name="pace_km" bind:value={() => pace1km, (v) => (time1seconds = v * distance1km)} unit={MIN_KM} />
    {:else}
		<TimeInput
      name="pace_mi"
			bind:value={
...
<div class="footer">
Show pace in <span class="unit-selection"><span class={paceUnits == MIN_MI ? "selected" : ""} onclick={paceUnits = MIN_MI}>min/mi</span> <span class={paceUnits == MIN_KM ? "selected" : ""} onclick={paceUnits = MIN_KM}>km/mi</span></span>
</div>
